...
<form class="formtable" style="padding-top: 60px;">
    <div class="col-sm-8">
            <h4 class="page-title"> Withdraw Monitor </h4>
            <ol class="breadcrumb">
...
function formatDate(dateString) {
      ...
      socket.onmessage = event => {
        ...
        if (response.length > 0) {
          ...
          $.each(response, function(index, row) {
            if (row.status !== 'Tahan') {
                            html += '<tr id="formWd_' + row.id + '" name="formDp">';
                            html += '<td style="padding: 0px; padding-top: 20px; font-size: 15px;">' + no + '</td>';
                            html += '<td style="padding: 0px; padding-top: 12px;"><button type="button" class="btn btn-light" onclick="copyRowData(' + row.id + ')""><i class="fas fa-copy" style="font-size: 24px; "></i></button></td>';
                            html += '<td style="padding: 0px; padding-top: 20px; cursor: pointer; font-size: 1em; font-weight: bold;" onclick="copyText(this, \'tanggal_masuk\')">' + formatDate(row.tanggal_masuk) + '</td>';
                            html += '<td style="padding: 0px; padding-top: 20px; cursor: pointer; font-size: 1em; font-weight: bold;" onclick="copyText(this, \'username\')">' + row.username + '</td>';
                            html += '<td style="padding: 0px; padding-top: 5px; cursor: pointer; font-size: 1em; font-weight: bold;" id="bankMemberWd" name="bankMemberWd" onclick="copyText(this, \'bank\')">';
                            ...
                            html += "<br>" + row.nama_bank + " " + "[" + row.norek + "]" + '</td>';

                            html += '<td style="padding: 0px; padding-top: 20px;">';
                            html += '<span class="nominal-asli" style="color: red; font-size: 1em; font-weight: bold;">' + formatRupiah1(row.nominal) + '</span>';
                            html += '</td>';
    ...
    toastr.success('Data from selected row has been copied.');
                                ...
                                function filterData(selectedBanks) {
                                    ...
                                    tableRows.forEach(function (row) {
                                        var bankColumn = row.querySelector("td[name='bankMemberWd']");
  ...
  function updateStatus(status, id, username, namaBank, bank, norek, nominal) {
    ...
    Swal.fire({
      title: "Ingin Memproses Withdraw Berikut?",
      html:
        "Username : " + username +
        "<br>Nama Bank : " + namaBank +
        "<br>Withdraw : " + formatRupiah(nominal),
      icon: "question",
      ...
      if (result.isConfirmed) {
        ...
        xhr.open("POST", "../admin_realtime/proses_form_wd.php", true);
  ...
  function processAll() {
    ...
    Swal.fire({
      title: "Apakah Anda Ingin Memproses Semua Yang Sudah DiTahan?",
      icon: "question",
      showCancelButton: true,
      confirmButtonText: "Proses",
      cancelButtonText: "Batal",
      ...
      if (result.isConfirmed) {
        ...
        var xhr = new XMLHttpRequest();
        xhr.open("POST", "../admin_realtime/proses_all_form_wd.php", true);
        xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
        xhr.onreadystatechange = function () {
          if (xhr.readyState === 4 && xhr.status === 200) {
            // Handle response
  ...
  function rejectRequest(id) {
    ...
    Swal.fire({
      title: "Masukkan Catatan Untuk Penolakan:",
      icon: "warning",
      input: "text",
      showCancelButton: true,
  ...
  function holdRequest(id) {
    ...
    var xhr = new XMLHttpRequest();
    xhr.open("POST", "../admin_realtime/tahan_status_wd.php", true);
    xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    xhr.onreadystatechange = function () {
      if (xhr.readyState === 4 && xhr.status === 200) {
