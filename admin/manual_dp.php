...
<form class="formtable" id="formDeposit" style="padding-top: 60px; width: 50%; align-items: center; justify-content: center; position: absolute; text-align: left; margin-left: 5px;">
  <div class="col-sm-8">
            <h4 class="page-title"> Form Deposit </h4>
            <ol class="breadcrumb">
                <li class="breadcrumb-item">Manual Deposit</li>
            </ol>
        </div>    
        <table class="table" style="border: 1px solid black;">
      <thead>
        <tr>
          <th class="table-dark" style="text-align: center;" colspan="3">MANUAL DEPOSIT</th>
        </tr>
      </thead>
      <tbody>
        <tr>
...
document.addEventListener('DOMContentLoaded', function() {
  document.getElementById('formDeposit').addEventListener('submit', function(event) {
    ...
    fetch('../admin_realtime/proses_manual_dp.php', {
      method: 'POST',
      body: formData
    })
    .then(response => {
    ...
    .then(data => {
      toastr.success('Berhasil Menambahkan Saldo.');
